...
const NewArrivals = () => {
  ...
  const next = () => {
    setStartIndex((prev) => (prev + 1) % (newProducts.length - itemsPerPage + 1))
  }

  const prev = () => {
    setStartIndex((prev) => (prev - 1 + (newProducts.length - itemsPerPage + 1)) % (newProducts.length - itemsPerPage + 1))
  }

  // Ensure we don't try to slice out of bounds if there are fewer items than itemsPerPage
  const safeStartIndex = newProducts.length <= itemsPerPage ? 0 : startIndex;
  const visibleProducts = newProducts.slice(safeStartIndex, safeStartIndex + itemsPerPage)
    ...
    <section className="py-12 bg-gray-50">
      <div className="container mx-auto px-6">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Yeni Məhsullar</h2>
            <p className="text-gray-500 text-sm">Ən son əlavə olunan məhsulları kəşf edin</p>
          </div>
