...
const DiscountedProducts = () => {
  ...
  const next = () => {
    setStartIndex((prev) => (prev + 1) % (discountedProducts.length - itemsPerPage + 1))
  }

  const prev = () => {
    setStartIndex((prev) => (prev - 1 + (discountedProducts.length - itemsPerPage + 1)) % (discountedProducts.length - itemsPerPage + 1))
  }

  // Ensure we don't try to slice out of bounds if there are fewer items than itemsPerPage
  const safeStartIndex = discountedProducts.length <= itemsPerPage ? 0 : startIndex;
  const visibleProducts = discountedProducts.slice(safeStartIndex, safeStartIndex + itemsPerPage)
    ...
    <section className="py-12 bg-white">
      <div className="container mx-auto px-6">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Endirimli Məhsullar</h2>
            <p className="text-gray-500 text-sm">Möhtəşəm endirimləri qaçırmayın</p>
          </div>
