    ...
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="bg-gray-50 border-b border-gray-100">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Link to="/" className="hover:text-gray-900">{t('common.home')}</Link>
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4">
            <div className="relative aspect-square bg-gray-50 rounded-2xl overflow-hidden flex items-center justify-center group border border-gray-100">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                {product.isNew && (
                  <span className="px-3 py-1.5 bg-blue-600 text-white text-xs font-bold tracking-wider uppercase rounded shadow-sm">{t('product.new')}</span>
                )}
                {product.isBestSeller && (
                  <span className="px-3 py-1.5 bg-amber-500 text-white text-xs font-bold tracking-wider uppercase rounded shadow-sm">{t('product.bestSeller')}</span>
                )}
                {product.discount && (
            ...
            <div className="mb-6 border-b border-gray-100 pb-6">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-sm font-medium text-blue-600 hover:underline cursor-pointer">
                  {product.reviewCount} {t('product.review')}
                </span>
                <span className="w-1 h-1 bg-gray-300 rounded-full"></span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-6 border-t border-gray-100">
                <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <div className="flex flex-col">
                        <span className="text-xs text-gray-500">{t('product.features.delivery.title')}</span>
                        <span className="text-sm font-bold">{t('product.features.delivery.value')}</span>
                    </div>
                </div>
                <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <div className="flex flex-col">
                        <span className="text-xs text-gray-500">{t('product.features.warranty.title')}</span>
                        <span className="text-sm font-bold">{t('product.features.warranty.value')}</span>
                    </div>
                </div>
                <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <div className="flex flex-col">
                        <span className="text-xs text-gray-500">{t('product.features.return.title')}</span>
                        <span className="text-sm font-bold">{t('product.features.return.value')}</span>
                    </div>
                </div>
            </div>
          </div>
                ...
                {activeTab === 'description' && (
                    <div className="prose max-w-none text-gray-600">
                        ...
                        <ul className="mt-4 list-disc pl-5 space-y-2">
                            ...
                            <li>{t('product.descriptionList.design')}</li>
                            <li>{t('product.descriptionList.usage')}</li>
                            <li>{t('product.descriptionList.cleaning')}</li>
                        </ul>
                    </div>
                )}
                {activeTab === 'specs' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                        <div className="flex justify-between py-3 border-b border-gray-100">
                            <span className="text-gray-500">{t('product.specs.brand')}</span>
                        ...
                        <div className="flex justify-between py-3 border-b border-gray-100">
                            ...
                            <span className="font-medium text-gray-900">{t('product.specs.allSeasons')}</span>
                        </div>
                    </div>
                )}
                {activeTab === 'reviews' && (
                    <div className="text-center py-12 text-gray-500">
                        <p>{t('product.reviews.empty')}</p>
                    </div>
                )}
            </div>
        </div>
        ...
        {similarProducts.length > 0 && (
          <div className="mt-20">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">{t('product.similarProducts')}</h2>
